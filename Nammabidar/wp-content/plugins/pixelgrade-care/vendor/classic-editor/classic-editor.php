<?php
/**
 * Classic Editor (with a minor addition by Pixelgrade)
 *
...
class Classic_Editor {
	...
	public static function init_actions() {
		...
		// Get the remote config and see if we have any instructions about Gutenberg.
		$config = PixelgradeCare_Admin::get_theme_config();

		// If the config tells us that the theme is Gutenberg ready, we really have nothing to do.
		if ( isset( $config['gutenbergReady'] ) && filter_var( $config['gutenbergReady'], FILTER_VALIDATE_BOOLEAN ) === true ) {
			return;
		}

		/*
		 * End of Pixelgrade addition!
		 * ============================
		 */
...
// Pixelgrade modification: hooking it to plugins_loaded is too early for the theme to have its say.
add_action( 'after_setup_theme', array( 'Classic_Editor', 'init_actions' ), 2 );
