...
/**
 * Class to handle searching and replacing strings in the database.
 *
 * Adapted from the Better Search Replace plugin by Delicious Brains: https://bettersearchreplace.com
 *
 * @since      1.15.0
 *
...
class PixelgradeCare_DbSearchReplace {

	/**
	 * The page size to use when processing table rows.
	 * @var int
	 */
	public $page_size;
	...
	public function __construct( $args = [] ) {
		...
		$this->page_size = ! empty( $args['page_size'] ) ? absint( $args['page_size'] ) : 30000;
	}
	...
	public static function get_sizes() {
		...
		if ( is_array( $tables ) && ! empty( $tables ) ) {
			...
			foreach ( $tables as $table ) {
				...
				$sizes[$table['Name']] = sprintf( __( '(%s MB)', 'pixelgrade_care' ), $size );
			}
	...
	public function srdb( $table, $page, $args ) {
		...
		if ( null === $primary_key ) {
			...
			return [ 'table_complete' => true, 'table_report' => $table_report ];
		}
		...
		foreach ( $data as $row ) {
			...
			foreach( $columns as $column ) {
				...
				if ( $this->wpdb->options === $table ) {
					...
					// Skip any options or caches of ours as they may contain the search field.
					if ( isset( $should_skip ) && true === $should_skip ) {
						$should_skip = false;
						continue;
					}

					if ( 'pixcare_options' === $data_to_fix
						 || 'style_manager_design_assets' === $data_to_fix
					     || 0 === strpos( $data_to_fix, 'pixelgrade_cloud_' )
					     || false !== strpos( $data_to_fix, '_transient_pixcare_theme_config_' )
					     || false !== strpos( $data_to_fix, '_transient_pixcare_license_products_' )
					     || false !== strpos( $data_to_fix, '_transient_pixcare_support_' )
					) {
						$should_skip = true;
					}
				}
			...
			if ( $args['dry_run'] === 'on' ) {
				...
				if ( ! $result ) {
					$table_report['errors'][] = sprintf( __( 'Error updating row: %d.', 'pixelgrade_care' ), $current_row );
				} else {
		...
		$this->wpdb->flush();
		return [ 'table_complete' => $done, 'table_report' => $table_report ];
	}
