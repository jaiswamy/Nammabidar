 ...
 */
class PixelgradeCare_MetafieldsCore {
	...
	static function instance() {
		...
		if ( isset( self::$mapping[ $interface ] ) ) {
			...
			// The interface isn't mapped to a class.
	 ...
	 * an interface the interface name with an Impl suffix is used.
	 */
	 ...
	 *
	 * @return PixelgradeCare_MetafieldsForm
	 */
	static function form( $config, $processor ) {
		$form = self::instance( 'PixelgradeCare_MetafieldsForm', $config );
	 ...
	 * @param array configuration
	 *
	 * @return PixelgradeCare_MetafieldsProcessor
	 */
	static function processor( $config ) {
		return self::instance( 'PixelgradeCare_MetafieldsProcessor', $config );
	}
	...
	/**
	 * Hierarchical array merge. Will always return an array.
	 *
	 * @param  ... arrays
	 *
	 * @return array
	 ...
	 * @throws Exception
	 */
	static function callback( $key, PixelgradeCare_MetafieldsMeta $meta ) {
		$defaults          = PixelgradeCare_MetafieldsCore::defaults();
		$default_callbacks = $defaults['callbacks'];
		$plugin_callbacks  = $meta->get( 'callbacks', [] );
	...
	protected static $textdomain = 'pixelgrade_care';
	...
	static function settextdomain( $textdomain ) {
		if ( ! empty( $textdomain ) ) {
			...
			// revert to default
			self::$textdomain = 'pixelgrade_care';
		}
	}
