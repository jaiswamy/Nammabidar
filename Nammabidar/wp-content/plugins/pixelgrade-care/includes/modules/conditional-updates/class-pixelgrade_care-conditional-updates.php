...
if ( ! class_exists( 'PixelgradeCare_Conditional_Updates' ) ) :

	class PixelgradeCare_Conditional_Updates {
		 ...
		 * @since   1.12.0
		 * @var     null|PixelgradeCare_Conditional_Updates
		 * @access  protected
		 */
		...
		public function notices() {
			...
			if ( ! empty( $screen )
			     && in_array( $screen->base, [ 'themes', 'update-core', 'update', 'plugins', 'dashboard' ] )
				...
				if ( ! empty( $failed_plugins = $this->check_current_active_plugins() ) ) {
					...
					add_action( 'admin_notices', function () use ( $failed_plugins ) {
						...
						printf(
							...
							esc_html__( 'Your Pixelgrade theme requires different plugin versions than the ones active:', 'pixelgrade_care' ),
							$failed_list,
							wp_kses_post( __( 'Please <strong>replace the current active plugin versions</strong> with the ones recommended for a smooth experience.<br><strong>The steps are as follows:</strong> download, go to <code>Plugins → Add New</code>, use the <code>Upload Plugin</code> button to select the downloaded zip file, and choose <code>Replace current with uploaded</code> when asked.', 'pixelgrade_care' ) )
						);
					} );
				}
			}
			...
			foreach ( $installed_plugins as $filename => $plugin_details ) {
				...
				if ( ! empty( $wporg_versions[ $found_version ] ) ) {
					...
					$failed[ $filename ] = sprintf(
						__( '<a href="%1$s" target="_blank">%2$s</a> - <strong>recommended version is %3$s;</strong> download it <a href="%4$s" target="_blank">here</a>;', 'pixelgrade_care' ),
		...
		/**
		 * Main PixelgradeCare_Conditional_Updates Instance
		 *
		 * Ensures only one instance of PixelgradeCare_Conditional_Updates is loaded or can be loaded.
		 *
		 * @since  1.12.0
		 * @static
		 *
		 ...
		 *
		 * @return PixelgradeCare_Conditional_Updates Main PixelgradeCare_Conditional_Updates instance
		 */
