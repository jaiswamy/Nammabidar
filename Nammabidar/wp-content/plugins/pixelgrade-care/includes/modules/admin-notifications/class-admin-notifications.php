...
/**
 *
 * A class to handle the behavior of pixcloud admin notices.
 *
 ...
 *
 */
class Pixcloud_Admin_Notifications {

	/**
	 * Holds the only instance of this class.
	 * @var     null|Pixcloud_Admin_Notifications
	 * @access  protected
	 * @since   1.9.0
	 */
	...
	public function init( array $args ) {
		...
		require_once 'class-admin-notifications-conditions.php';
	}
	...
	public function add_hooks() {
		...
		add_action( 'admin_init', [ $this, 'maybe_load_remote_notifications' ] );
		...
		add_action( 'wp_ajax_' . $this->manager_id . '_dismiss_admin_notice', [ $this, 'dismiss_notice' ] );

		// Add notifications data to requests.
		add_filter( 'pixelgrade_cloud_request_data', [ $this, 'add_data_to_request' ], 10, 1 );
		add_filter( 'customify_pixelgrade_cloud_request_data', [ $this, 'add_data_to_request' ], 10, 1 );
	}
	...
	public function get_remote_notifications_config( $skip_cache = false ) {
		// Make sure that the Notifications class is loaded.
		require_once 'lib/class-notifications.php';

		// Get the notifications data.
		$remote_data = Pixcloud_Notifications::instance()->get( $skip_cache );
		if ( false === $remote_data || empty( $remote_data['items'] ) ) {
			$notifications_config = [];
		} else {
			$notifications_config = $remote_data['items'];
		}

		return apply_filters( 'pixcloud_admin_notifications_get_remote_notifications', $notifications_config );
	}
	...
	public function enqueue_scripts( $hook ) {
		...
		wp_enqueue_script( 'pixcloud-admin-notifications-js', $this->url_to_assets_dir . 'admin-notifications.js', [ 'jquery' ], $this->version );
		wp_enqueue_style( 'pixcloud-admin-notifications-css', $this->url_to_assets_dir . 'admin-notifications.css', [], $this->version );
	}
	...
	public function dismiss_on_redirect_link( array $args ) {
		...
		$defaults = [
			...
			'redirect' => '',
			'new_tab'  => false,
			'classes'  => [ 'pixcloud_anm-link' ],
		];

		$args = wp_parse_args( $args, $defaults );

		$classes = [ 'pixcloud_anm-dismiss' ];
		...
		return '<button type="submit" class="' . esc_attr( $classes ) . '" name="pixcloud_anm-redirect" data-newtab="' . esc_attr( intval( $args['new_tab'] ) ) . '" value="' . esc_attr( $args['redirect'] ) . '">' . $args['content'] . '</button>';
	...
	public function dismiss_event_button( array $args ) {
		...
		$classes = [ 'pixcloud_anm-dismiss', 'pixcloud_anm-event' ];
		if ( is_array( $args['classes'] ) ) {
			$classes = array_merge( $args['classes'], $classes );
		}
		$classes = implode( ' ', $classes );
		...
		return '<button type="submit" class="' . esc_attr( $classes ) . '" name="pixcloud_anm-event" value="' . esc_attr( esc_attr( $args['event'] ) ) . '">' . $args['content'] . '</button>';
		...
		if ( ! empty( $notices ) ) {
			...
			foreach ( $notices as $notice_id => $notice ) {
				...
				if ( ! empty( $notice['local_conditions'] ) ) {
					$display = Pixcloud_Admin_Notifications_Conditions::process( $notice['local_conditions'] );
				}
	...
	protected function display_standard_notice( $notice ) {
		...
		?>
		<div id="<?php echo esc_attr( $this->manager_id . '-' . $notice['id'] ); ?>" class="<?php echo esc_attr( implode( ' ', $container_classes ) ); ?>">
			<form class="pixcloud_anm-form"
			      action="<?php echo esc_url( admin_url( 'admin-ajax.php?action=' . $this->manager_id . '_dismiss_admin_notice' ) ); ?>"
			      method="post">
				...
				if ( $notice['no_js_dismissable'] ) {
					...
					<noscript>
						...
						<td>
							<button type="submit" class="notice-dismiss"><span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'pixelgrade_care' ); ?></span>
							</button>
						</td>
						</tr></table>
					</noscript>
			...
			<form class="pixcloud_anm-form"
				...
				if ( $notice['no_js_dismissable'] ) {
					...
					<noscript>
						...
						<td>
							<button type="submit" class="notice-dismiss"><span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'pixelgrade_care' ); ?></span></button>
						</td>
						</tr></table>
					</noscript>
				<?php } ?>
	...
	protected function parse_content_tags( $content ) {
		...
		$content = apply_filters( 'pixcloud_admin_notifications_before_parse_content_tags', $content );
		...
		// %active_theme%
		$content = str_replace( '%active_theme%', Pixcloud_Admin_Notifications_Conditions::get_active_theme_name(), $content );

		// %customify_version%
		$content = str_replace( '%customify_version%', Pixcloud_Admin_Notifications_Conditions::get_customify_version(), $content );

		// %style_manager_version%
		$content = str_replace( '%style_manager_version%', Pixcloud_Admin_Notifications_Conditions::get_style_manager_version(), $content );

		// %current_color_palette%
		$content = str_replace( '%current_color_palette%', Pixcloud_Admin_Notifications_Conditions::get_current_color_palette_label(), $content );
		...
		return apply_filters( 'pixcloud_admin_notifications_after_parse_content_tags', $content, $original_content );
	}
	...
	/**
	 * Main Pixcloud_Admin_Notifications Instance
	 *
	 * Ensures only one instance of Pixcloud_Admin_Notifications is loaded or can be loaded.
	 *
	 * @since  1.9.0
	 * @static
	 *
	 * @param array $args The arguments to initialize the notifications manager.
	 * @return Pixcloud_Admin_Notifications Main Pixcloud_Admin_Notifications instance
	 */
