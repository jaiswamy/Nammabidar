...
function pixcare_disable_customify_notifications() {
	if ( class_exists( 'Pixcloud_Admin_Notifications_Manager' ) ) {
		...
		if ( ! empty( $instance ) ) {
			remove_action( 'admin_init', [ $instance, 'maybe_load_remote_notifications' ] );
			remove_action( 'admin_notices', [ $instance, 'display_notices' ] );
			remove_action( 'admin_enqueue_scripts', [ $instance, 'enqueue_scripts' ] );
			remove_action( 'wp_ajax_' . 'pixcloud_anm' . '_dismiss_admin_notice', [ $instance, 'dismiss_notice' ] );
			remove_filter( 'customify_pixelgrade_cloud_request_data', [ $instance, 'add_data_to_request' ], 10 );
		}
