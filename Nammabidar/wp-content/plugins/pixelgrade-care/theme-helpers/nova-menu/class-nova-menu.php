...
class Pixcare_Nova_Menu {
	...
	public function register_hooks() {
		add_action( 'init', [ $this, 'add_shortcodes' ], 10 );

		// Initialize the Jetpack Nova_Restaurant class with our settings for markup
		add_action( 'wp_head', [ $this, 'nova_restaurant_init' ] );
	...
	public function nova_menu_shortcode( $atts ) {
		...
		if ( false == $atts['display_sections'] ) {
			add_filter( 'jetpack_nova_menu_item_loop_open_element' , [ $this, 'menu_item_open_hidden' ], 10, 4 );
			add_filter( 'jetpack_nova_menu_item_loop_close_element' , [ $this, 'menu_item_close_hidden' ], 10, 4 );
		} else {
			// make sure that we wrap each section items in `<div class="menu-list__items">`
			add_filter( 'jetpack_nova_menu_item_loop_close_element' , [ $this, 'menu_item_close_wrap_section_items' ], 10, 4 );
		}
		...
		add_action( 'loop_start', [ $instance, 'start_menu_item_loop' ] );

		$output = $this->nova_menu_shortcode_html( $atts );

		// Remove the filters in case we've added them
		if ( false == $atts['display_sections'] ) {
			remove_filter( 'jetpack_nova_menu_item_loop_open_element' , [ $this, 'menu_item_open_hidden' ], 10 );
			remove_filter( 'jetpack_nova_menu_item_loop_close_element' , [ $this, 'menu_item_close_hidden' ], 10 );
		} else {
			remove_filter( 'jetpack_nova_menu_item_loop_close_element' , [ $this, 'menu_item_close_wrap_section_items' ], 10 );
		}
	...
	public function menu_item_open_hidden( $tag, $field, $markup, $term ) {
		...
		$menu_tag = $field . '_tag';
		if ( in_array( $menu_tag, [ 'menu_tag', 'menu_header_tag' ] ) ) {
			return '';
		}
	...
	public function get_menu_item_class( $post_id, $menu_item_index_number, $atts ) {
		...
		$menu_item_sections = wp_get_object_terms( $post_id, self::MENU_SECTION_TAX, [ 'fields' => 'slugs' ] );
	 ...
	 * @return string
	 */
	public function the_menu_item_labels( $post_id, $exclude_labels = [] ) {
		$menu_item_labels = get_the_terms( $post_id, self::MENU_ITEM_LABEL_TAX );
	 ...
	 */
	public static function instance(  $menu_item_loop_markup = [] ) {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'You should not do that!', 'pixelgrade_care' ), esc_html( $this->_version ) );
	} // End __clone ()
