...
// This way Jetpack doesn't burn us down with its lack of checking if the class exists
if ( ! empty( $action ) && in_array( $action, [ 'activate', 'activate-plugin', 'tgmpa-bulk-activate' ] ) ) {
	return;
}
...
if ( ! class_exists( 'Jetpack_Portfolio' ) ) {
	...
	class Jetpack_Portfolio {
		...
		function __construct() {
			...
			add_action( 'after_switch_theme', [ $this, 'activation_post_type_support' ] );

			// Make sure the post types are loaded for imports
			add_action( 'import_start', [ $this, 'register_post_types' ] );

			// Add to REST API post type whitelist
			add_filter( 'rest_api_allowed_post_types', [ $this, 'allow_portfolio_rest_api_type' ] );
			...
			add_action( sprintf( 'add_option_%s', self::OPTION_NAME ), [ $this, 'flush_rules_on_enable' ], 10 );
			add_action( sprintf( 'update_option_%s', self::OPTION_NAME ), [ $this, 'flush_rules_on_enable' ], 10 );
			add_action( sprintf( 'publish_%s', self::CUSTOM_POST_TYPE ), [
				$this,
				'flush_rules_on_first_project'
			] );
			add_action( 'after_switch_theme', [ $this, 'flush_rules_on_switch' ] );
			...
			add_shortcode( 'jetpack_portfolio', [ $this, 'portfolio_shortcode' ] );
		...
		function settings_api_init() {
			add_settings_field(
				self::OPTION_NAME,
				'<span class="cpt-options">' . esc_html__( 'Portfolio Projects', 'pixelgrade_care' ) . '</span>',
				[ $this, 'setting_html' ],
				'writing',
				'jetpack_cpt_section'
			);
		...
		function setting_html() {
			if ( current_theme_supports( self::CUSTOM_POST_TYPE ) ) : ?>
				<p><?php printf( wp_kses_post( __( 'Your theme supports <strong>%s</strong>', 'pixelgrade_care' ) ), self::CUSTOM_POST_TYPE ); ?></p>
			<?php else : ?>
			...
			if ( get_option( self::OPTION_NAME, '0' ) || current_theme_supports( self::CUSTOM_POST_TYPE ) ) :
				printf( '<p><label for="%1$s">%2$s</label></p>',
					...
					sprintf( esc_html__( 'Portfolio pages display at most %1$s projects', 'pixelgrade_care' ),
		...
		function updated_messages( $messages ) {
			...
			$messages[ self::CUSTOM_POST_TYPE ] = [
				...
				1  => sprintf( wp_kses_post( __( 'Project updated. <a href="%s">View item</a>', 'pixelgrade_care' ) ), esc_url( get_permalink( $post->ID ) ) ),
				...
				6  => sprintf( wp_kses_post( __( 'Project published. <a href="%s">View project</a>', 'pixelgrade_care' ) ), esc_url( get_permalink( $post->ID ) ) ),
				7  => esc_html__( 'Project saved.', 'pixelgrade_care' ),
				8  => sprintf( wp_kses_post( __( 'Project submitted. <a target="_blank" href="%s">Preview project</a>', 'pixelgrade_care' ) ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
				9  => sprintf( wp_kses_post( __( 'Project scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview project</a>', 'pixelgrade_care' ) ),
					// translators: Publish box date format, see http://php.net/date
					date_i18n( esc_html__( 'M j, Y @ G:i', 'pixelgrade_care' ), strtotime( $post->post_date ) ), esc_url( get_permalink( $post->ID ) ) ),
				10 => sprintf( wp_kses_post( __( 'Project item draft updated. <a target="_blank" href="%s">Preview project</a>', 'pixelgrade_care' ) ), esc_url( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ),
			];
		...
		function edit_admin_columns( $columns ) {
			...
			$columns['title'] = esc_html__( 'Project', 'pixelgrade_care' );
		...
		function customize_register( $wp_customize ) {
			...
			$wp_customize->add_section( 'jetpack_portfolio', [
			...
			if ( isset( $options[0]['title'] ) && true === $options[0]['title'] ) {
				$wp_customize->add_setting( 'jetpack_portfolio_title', [
				...
				$wp_customize->add_control( 'jetpack_portfolio_title', [
			...
			if ( isset( $options[0]['content'] ) && true === $options[0]['content'] ) {
				$wp_customize->add_setting( 'jetpack_portfolio_content', [
					'default'              => '',
					'type'                 => 'option',
			...
			if ( isset( $options[0]['featured-image'] ) && true === $options[0]['featured-image'] ) {
				$wp_customize->add_setting( 'jetpack_portfolio_featured_image', [
					'default'              => '',
					'type'                 => 'option',
				...
				$wp_customize->add_control( new WP_Customize_Image_Control( $wp_customize, 'jetpack_portfolio_featured_image', [
					'section' => 'jetpack_portfolio',
		...
		static function portfolio_shortcode( $atts ) {
			...
			if ( $atts['orderby'] ) {
				...
				$allowed_keys    = [ 'author', 'date', 'title', 'rand' ];
			...
			wp_enqueue_style( 'jetpack-portfolio-style', plugins_url( 'css/portfolio-shortcode.css', __FILE__ ), [], '20140326' );
		...
		static function portfolio_shortcode_html( $atts ) {
			...
			if ( $query->have_posts() ) {
				...
				<p>
					<em><?php esc_html_e( 'Your Portfolio Archive currently has no entries. You can start creating them on your dashboard.', 'pixelgrade_care' ); ?>
				</p></em>
				<?php
			}
			$html = ob_get_clean();
		...
		static function get_project_class( $portfolio_index_number, $columns ) {
			$project_types = wp_get_object_terms( get_the_ID(), self::CUSTOM_TAXONOMY_TYPE, [ 'fields' => 'slugs' ] );
			$class         = [];
		...
		static function get_project_type( $post_id ) {
			...
			$html  = '<div class="project-types"><span>' . esc_html__( 'Types', 'pixelgrade_care' ) . ':</span>';
			$types = [];
		...
		static function get_project_tags( $post_id ) {
			...
			$html = '<div class="project-tags"><span>' . esc_html__( 'Tags', 'pixelgrade_care' ) . ':</span>';
			$tags = [];
		...
		static function get_project_author() {
			...
			$html .= sprintf( wp_kses_post( __( '<span>Author:</span> <a href="%1$s">%2$s</a>', 'pixelgrade_care' ) ),
	...
	add_action( 'init', [ 'Jetpack_Portfolio', 'init' ] );

	// Check on plugin activation if theme supports CPT
	register_activation_hook( __FILE__, [ 'Jetpack_Portfolio', 'activation_post_type_support' ] );
