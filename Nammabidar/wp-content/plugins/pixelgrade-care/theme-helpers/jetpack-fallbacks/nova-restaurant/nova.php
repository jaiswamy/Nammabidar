...
if ( ! class_exists( 'Nova_Restaurant' ) ) {
	...
	*/
	...
	class Nova_Restaurant {
		...
		static function init( $menu_item_loop_markup = [] ) {
		...
		function __construct() {
			...
			add_action( 'admin_enqueue_scripts', [ $this, 'enqueue_nova_styles' ] );
			add_action( 'admin_head', [ $this, 'set_custom_font_icon' ] );

			// Always sort menu items correctly
			add_action( 'parse_query', [ $this, 'sort_menu_item_queries_by_menu_order' ] );
			add_filter( 'posts_results', [ $this, 'sort_menu_item_queries_by_menu_taxonomy' ], 10, 2 );

			add_action( 'wp_insert_post', [ $this, 'add_post_meta' ] );
			...
			add_filter( 'template_include', [ $this, 'setup_menu_item_loop_markup__in_filter' ] );

			add_filter( 'enter_title_here', [ $this, 'change_default_title' ] );
			add_filter( 'post_updated_messages', [ $this, 'updated_messages' ] );
			add_filter( 'dashboard_glance_items', [ $this, 'add_to_dashboard' ] );
		}
		...
		function register_taxonomies() {
			if ( ! taxonomy_exists( self::MENU_ITEM_LABEL_TAX ) ) {
				register_taxonomy( self::MENU_ITEM_LABEL_TAX, self::MENU_ITEM_POST_TYPE, [
					'labels'       => [
			...
			if ( ! taxonomy_exists( self::MENU_TAX ) ) {
				register_taxonomy( self::MENU_TAX, self::MENU_ITEM_POST_TYPE, [
					'labels'        => [
						/* translators: this is about a food menu */
		...
		function register_post_types() {
			...
			register_post_type( self::MENU_ITEM_POST_TYPE, [
				...
				'register_meta_box_cb' => [ $this, 'register_menu_item_meta_boxes' ],
		...
		function updated_messages( $messages ) {
			...
			$messages[ self::MENU_ITEM_POST_TYPE ] = [
		...
		function enqueue_nova_styles( $hook ) {
			global $post_type;
			$pages = [ 'edit.php', 'post.php', 'post-new.php' ];

			if ( in_array( $hook, $pages ) && $post_type == self::MENU_ITEM_POST_TYPE ) {
				wp_enqueue_style( 'nova-style', plugins_url( 'css/nova.css', __FILE__ ), [], $this->version );
			}

			wp_enqueue_style( 'nova-font', plugins_url( 'css/nova-font.css', __FILE__ ), [], $this->version );
		}
		...
		function sort_menu_item_queries_by_menu_taxonomy( $posts, $query ) {
			...
			foreach ( $posts as $post ) {
				...
				if ( ! isset( $grouped_by_term["$term_id"] ) ) {
					$grouped_by_term["$term_id"] = [];
				}

				$grouped_by_term["$term_id"][] = $post;
			}

			$term_order = get_option( 'nova_menu_order', [] );
		...
		function add_admin_menus() {
			$hook = add_submenu_page(
				...
				'edit_pages',
				'add_many_nova_items',
				[ $this, 'add_many_new_items_page' ]
			);

			add_action( "load-$hook", [ $this, 'add_many_new_items_page_load' ] );

			add_action( 'current_screen', [ $this, 'current_screen_load' ] );
		...
		function admin_notices() {
			if ( isset( $_GET['nova_reordered'] ) ) /* translators: this is about a food menu */ {
		...
		function setup_menu_item_columns() {
			add_filter( sprintf( 'manage_edit-%s_sortable_columns', self::MENU_ITEM_POST_TYPE ), [
				$this,
				'no_title_sorting'
			] );
			add_filter( sprintf( 'manage_%s_posts_columns', self::MENU_ITEM_POST_TYPE ), [
				$this,
				'menu_item_columns'
			] );

			add_action( sprintf( 'manage_%s_posts_custom_column', self::MENU_ITEM_POST_TYPE ), [
				$this,
		...
		function edit_menu_items_page_load() {
			...
			wp_localize_script( 'nova-drag-drop', '_novaDragDrop', [
			...
			add_action( 'the_post', [ $this, 'show_menu_titles_in_menu_item_list' ] );
		}
			...
			<tr class="no-items menu-label-row" data-term_id="<?php echo esc_attr( $term_id ) ?>">
				...
				<td>
					<?php if ( $term instanceof WP_Term ) { ?>
						...
						<br/>
						<a class="nova-move-menu-down"
						   title="<?php esc_attr_e( 'Move menu section down', 'jetpack' ); ?>"
		...
		function add_many_new_items_page() {
			...
			<div class="wrap">
				...
				<form method="post" action="" enctype="multipart/form-data">
					<p>
					<h3><?php esc_html_e( 'Add to section:', 'jetpack' ); ?><?php wp_dropdown_categories( [
							'id'           => 'nova-menu-tax',
							'name'         => 'nova_menu_tax',
							'taxonomy'     => self::MENU_TAX,
							'hide_empty'   => false,
		...
		function register_menu_item_meta_boxes() {
			...
			add_meta_box( 'menu_item_price', __( 'Price', 'jetpack' ), [
				$this,
		...
		function get_menu_item_menu_leaf( $post_id ) {
			...
			$term_ids = wp_get_object_terms( $post_id, self::MENU_TAX, [ 'fields' => 'ids' ] );
		...
		function setup_menu_item_loop_markup__in_filter( $template ) {
			add_action( 'loop_start', [ $this, 'start_menu_item_loop' ] );
		...
		function start_menu_item_loop( $query ) {
			...
			add_action( 'the_post', [ $this, 'menu_item_loop_each_post' ] );
			add_action( 'loop_end', [ $this, 'stop_menu_item_loop' ] );
		}
		...
		function stop_menu_item_loop( $query ) {
			...
			remove_action( 'the_post', [ $this, 'menu_item_loop_each_post' ] );
			remove_action( 'loop_start', [ $this, 'start_menu_item_loop' ] );
			remove_action( 'loop_end', [ $this, 'stop_menu_item_loop' ] );
	...
	add_action( 'init', [ 'Nova_Restaurant', 'init' ] );
}
