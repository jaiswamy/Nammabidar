...
class PixJetpackCustomization {
	...
	function register_hooks() {
		add_filter( 'default_option_jetpack_active_modules', [ $this, 'default_jetpack_active_modules' ], 10, 1 );
		add_filter( 'jetpack_get_available_modules', [ $this, 'jetpack_hide_blocked_modules' ], 20, 1 );
		add_filter( 'default_option_sharing-options', [ $this, 'default_jetpack_sharing_options' ], 10, 1 );
	}
	...
	function default_jetpack_active_modules( $default ) {
		...
		$jetpack_default_modules = [];

		//Read the options saved in the DB by the theme
		$theme_default_modules = get_theme_mod( 'pixelgrade_jetpack_default_active_modules', [] );
	...
	function default_jetpack_sharing_options( $default ) {
		...
		$jetpack_sharing_default_options = apply_filters ( 'pixelgrade_filter_jetpack_sharing_default_options', [] );
	...
	function jetpack_hide_blocked_modules( $modules ) {
		...
		$jetpack_blocked_modules = [];

		//Read the options saved in the DB by the theme
		$theme_blocked_modules = get_theme_mod( 'pixelgrade_jetpack_blocked_modules', [] );
