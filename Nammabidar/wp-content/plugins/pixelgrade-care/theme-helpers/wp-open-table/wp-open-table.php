...
if ( ! class_exists( 'Pixcare_Open_Table_Widget' ) ) :
class Pixcare_Open_Table_Widget extends WP_Widget {
		...
		if ( ! empty( $open_table_embed_code ) ) {
			...
			printf( __( '<p>You first need to copy&paste your EMBED code from <a href="%s">OpenTable Reservations</a> in <a href="%s">Appearance > Customize > General</a>.</p>', 'pixelgrade_care' ),
				...
				esc_url( add_query_arg(
					[
						[ 'autofocus' => [ 'section' => 'pixelgrade_options[general]' ] ],
	...
	static public function extract_embed_params( $js_string ) {
		...
		$parts['query'] = pixcare_open_table_proper_parse_str( urldecode( $parts['query'] ) );
...
if ( ! function_exists ( 'pixcare_open_table_proper_parse_str' ) ) :
function pixcare_open_table_proper_parse_str($str) {
	# result array
	$arr = [];
...
function pixelgrade_open_table_customify_option( $options ) {
	...
	$options['general']['options']['open_table_embed_code'] = [
		'type'    => 'textarea',
...
function pixelgrade_ot_reservation_widget_shortcode( $atts ) {
	...
	if ( $atts['type'] && ! in_array( $atts['type'], [ 'standard', 'tall', 'wide', 'button' ] ) ) {
	...
	if ( ! empty( $open_table_embed_code ) ) {
		// we need to process it a bit - extract the parameters
		$params = Pixcare_Open_Table_Widget::extract_embed_params( $open_table_embed_code );
		...
		// Now rebuild the parameters with out special alterations depending on widget options
		return '<script type="text/javascript" src="' . esc_attr( Pixcare_Open_Table_Widget::build_embed_url( $params ) ) . '"></script>';
	} else {
