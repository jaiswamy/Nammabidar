...
function pixelgrade_load_theme_dependent_functionality() {
	...
	if ( current_theme_supports( 'pixelgrade_opentable_widget' ) ) {
		...
		// Register the widget
		add_action( 'widgets_init', 'pixcare_register_open_table_widget' );
	}
	...
	if ( $should_load_jetpack_fallbacks ) {
		...
		if ( current_theme_supports( 'pixelgrade_jetpack_portfolio_shortcode' ) ) {
			...
			add_action( 'init', [ 'Pixelgrade_Jetpack_Portfolio_Shortcode', 'instance' ], 12 );
		}
	}
...
function pixcare_register_open_table_widget() {
	register_widget( 'Pixcare_Open_Table_Widget' );
}
